...
class Isolate extends Widget_Base {
	...
	public function get_script_depends()
	{
		if (bdt_ps()->is__premium_only()) {
			return ['bdt-uikit-icons', 'gsap', 'split-text', 'ps-isolate'];
		} else {
			...
			[
				...
				'default' => [
					[
						...
						'image' => ['url' => BDTPS_ASSETS_URL . 'images/gallery/img-4.svg']
					],
					[
						'title' => esc_html__('Slide Item 2', 'bdthemes-prime-slider'),
						'image' => ['url' => BDTPS_ASSETS_URL . 'images/gallery/img-5.svg']
					],
					[
						'title' => esc_html__('Slide Item 3', 'bdthemes-prime-slider'),
						'image' => ['url' => BDTPS_ASSETS_URL . 'images/gallery/img-6.svg']
					],
				],
				'title_field' => '{{{ title }}}',
			]
	...
	public function render_header($skin_name = 'isolate') {
		...
		?>
			<div class="bdt-prime-slider">
				...
				<?php if( 'yes' == $settings['show_logo'] or 'yes' == $settings['show_menu'] or 'yes' == $settings['show_offcanvas'] ) : ?>
				<div class="bdt-header-wrapper bdt-position-top">
					<header <?php $this->print_render_attribute_string('header'); ?>>
				...
				<a <?php $this->print_render_attribute_string('button'); ?>>
					...
					<span <?php $this->print_render_attribute_string('content-wrapper'); ?>>
						...
						<?php if (!empty($settings['offcanvas_button_text'])) : ?>
							<span <?php $this->print_render_attribute_string('text'); ?>><?php echo wp_kses($settings['offcanvas_button_text'], prime_slider_allow_tags('title')); ?></span>
						<?php endif; ?>
					</span>
	...
	protected function render_play_button($slide) {
		...
		if ( 'shadow-pulse' == $settings['fancy_animation'] ) {
			$this->add_render_attribute( 'lightbox', 'class', 'bdt-slide-play-button bdt-position-center bdt-shadow-pulse', true );
		} elseif ( 'line-bounce' == $settings['fancy_animation'] ) {
			$this->add_render_attribute( 'lightbox', 'class', 'bdt-slide-play-button bdt-position-center bdt-line-bounce', true );
		} elseif ( 'multi-shadow' == $settings['fancy_animation'] ) {
			$this->add_render_attribute( 'lightbox', 'class', 'bdt-slide-play-button bdt-position-center bdt-multi-shadow', true );
		} else {
                        ...
                        <div class="bdt-prime-slider-desc">
							...
							<?php if ($slide_content['sub_title'] && ('yes' == $settings['show_sub_title'])) : ?>
                                <div class="bdt-sub-title bdt-ps-sub-title">
                                    <h4 <?php echo $parallax_sub_title; ?>>
                            ...
                            <?php if ($slide_content['excerpt'] && ('yes' == $settings['show_excerpt'])) : ?>
                                <div class="bdt-slider-excerpt" <?php echo $parallax_excerpt;?>>
