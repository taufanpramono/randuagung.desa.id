(function ($, elementor) {
    ...
    var widgetCrossroad = function ($scope, $) {
        ...
        class Slide {
            ...
            setCenter() {
                ...
                TweenMax.set([this.DOM.el, this.DOM.title], {
                    opacity: 1
                });
            }
            setRight() {
            ...
            reset() {
                TweenMax.set([this.DOM.el, this.DOM.imgWrap, this.DOM.number, this.DOM.subtitle, this.DOM.title], {
                    transform: 'none'
                });
                TweenMax.set([this.DOM.el, this.DOM.title], {
                    opacity: 0
            ...
            animateElementsOut(contentItem) {
                return new Promise((resolve, reject) => {
                    ...
                    this.elemsTimeline = new TimelineMax({
                            onComplete: resolve
                        })
        ...
        class Slideshow {
            constructor(el, id) {
                ...
                this.elementID = id;
                // The titles
                this.DOM.titlesWrap = this.DOM.el.querySelector(id + ' .titles-wrap');
                this.DOM.titlesInner = this.DOM.titlesWrap.querySelector(id + ' .grid--titles');
                this.DOM.titles = [...this.DOM.titlesInner.querySelectorAll(id + ' .grid__item--title')];
                // The slides instances
                this.slides = [];
                [...this.DOM.el.querySelectorAll(id + ' .grid__item--slide')].forEach((slide, pos) => this.slides.push(new Slide(slide, this.DOM.titles[pos])));
                ...
                // Content Items
                this.DOM.contentItems = [...document.querySelectorAll(id + ' .content__item')];

                // Areas (left, center, right) where to attach the navigation events.
                this.DOM.interaction = {
                    left: document.querySelector(id + ' .grid__item--left'),
                    center: document.querySelector(id + ' .grid__item--center'),
                    right: document.querySelector(id + ' .grid__item--right')
                };
            ...
            initEvents() {
                ...
                this.DOM.contentItems.forEach(item => {
                    item.querySelector(this.elementID + ' .img-wrap--content').addEventListener('click', () => this.closeSlide());
                });
            }
            navigate(direction) {
                if (this.isAnimating) {
        ...
        var $widgetCrossroad = $scope.find('.bdt-crossroad-slider'),
            $settings = $widgetCrossroad.data('settings');
        if (!$widgetCrossroad.length) {
            return;
        }

        // Revealer element
        const revealer = new Revealer(document.querySelector($settings.id + ' .revealer__inner'));
        // Initialize the slideshow
        new Slideshow(document.querySelector($settings.id + ' .grid--slideshow'), $settings.id);
    ...
    jQuery(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/prime-slider-crossroad.default', widgetCrossroad);
    });
