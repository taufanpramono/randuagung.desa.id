...
namespace PrimeSlider\Modules\Crossroad\Widgets;
...
class Crossroad extends Widget_Base {
	...
	public function get_icon() {
		return 'bdt-widget-icon ps-wi-crossroad bdt-new';
	}
	...
	private function register_query_section_controls() {
		...
		$repeater->add_control(
			'image',
			[
				'label'     => esc_html__('Thumbnail', 'bdthemes-prime-slider'),
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-crossroad-slider .grid__item--title' => 'color: {{VALUE}}; -webkit-text-stroke-color: {{VALUE}};',
				],
				'condition' => [
					'show_title' => 'yes',
				],
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .bdt-crossroad-slider .grid__item--title' => '-webkit-text-stroke-width: {{SIZE}}px;',
				],
				'condition' => [
					'show_title' => 'yes',
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .bdt-crossroad-slider .content__item-header-title' => 'color: {{VALUE}}; -webkit-text-stroke-color: {{VALUE}};',
				],
				'condition' => [
					'show_title' => 'yes',
				],
		...
		$this->add_control(
			'modal_title_stroke_width',
			[
				'label'     => esc_html__('Stroke Width', 'bdthemes-prime-slider'),
				'type'      => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .bdt-crossroad-slider .content__item-header-title' => '-webkit-text-stroke-width: {{SIZE}}px;',
				],
				'condition' => [
					'show_title' => 'yes',
				],
	...
	public function render_title($slide) {
		...
		?>
		<<?php echo Utils::get_valid_html_tag($settings['title_html_tag']); ?> class="content__item-header-title">
	...
	public function rendar_item_image($item, $alt = '') {
		...
		<div class="img" style="background-image: url('<?php echo esc_url($image_final_src); ?>')"></div>
	...
	public function render_slides_loop() {
        ...
        foreach ($settings['slides'] as $slide) : ?>
			...
			<figure class="grid__item grid__item--slide <?php //echo ($i == 0) ? ' grid__item--slide' : ''; ?>">
				<?php if ($settings['show_counter'] == 'yes') : ?>
				<span class="number"><?php printf("%02d", $slide_index); ?></span>
				<?php endif; ?>
				...
				<?php if ($slide['meta_text'] && ('yes' == $settings['show_meta_text'])) : ?>
				<figcaption class="caption"><?php echo wp_kses_post($slide['meta_text']) ?></figcaption>
				<?php endif; ?>
			</figure>
		<?php 
		// $i++;
	...
	public function render_title_loop() {
		...
		<<?php echo Utils::get_valid_html_tag($settings['title_html_tag']); ?> class="grid__item grid__item--title">
